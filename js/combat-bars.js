...
export class CombatBars {
    static init() {
        ...
        Hooks.on('renderTokenConfig', function (app, html, options) {
            ...
            let value = (app.object instanceof TokenDocument ? app.object.getFlag('monks-little-details', 'displayBarsCombat') : getProperty(app.object.token, "flags.monks-little-details.displayBarsCombat"));
        ...
        Hooks.on("refreshToken", (token) => {
            ...
            if (MonksLittleDetails.isDefeated(token))
                return;

            if (token.inCombat) {
                let combatBar = token.document.getFlag('monks-little-details', 'displayBarsCombat');
        ...
        let tokenDrawBars = function (wrapped, ...args) {
            if (this.inCombat && this.document.displayBars === CONST.TOKEN_DISPLAY_MODES.NONE && this.document.flags['monks-little-details']?.displayBarsCombat !== CONST.TOKEN_DISPLAY_MODES.NONE) {
